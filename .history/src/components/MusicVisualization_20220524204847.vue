...
export default {
    ...
    mounted() {
        this.$store.commit("getGlobalVisualization", this.musicVisualization);
        // this.$store.commit("getGlobalCanvas", this.$refs.canvas);
    },
    methods: {
        visualizerSelected() {
            // 更新当前可视化选项
        ...
        init() {
            ...
            // this.$store.state.globalPlayer.play();
        ...
        musicVisualization() {
            // Error Uncaught DOMException: Failed to execute 'createMediaElementSource' on 'AudioContext': HTMLMediaElement already connected previously to a different MediaElementSourceNode.
            // chromium bug: https://bugs.chromium.org/p/chromium/issues/detail?id=851310
            ...
            // 获取 audio DOM 元素
            // TODO const currentAudio = this.$refs.audio;
            // 获取 vuex store.globalPlayer
            const currentAudio = this.$store.state.globalPlayer;
            // this.$store.state.globalPlayer.play();
            ...
            // if (MEDIA_ELEMENT_NODES.has(currentAudio)) {
            //     const source = MEDIA_ELEMENT_NODES.get(currentAudio);
            // } else {
            //     const source = audioCtx.createMediaElementSource(currentAudio);
            //     MEDIA_ELEMENT_NODES.set(currentAudio, source);
            // }

            let source = audioCtx.createMediaElementSource(currentAudio);
            // 将音频源关联到分析器
