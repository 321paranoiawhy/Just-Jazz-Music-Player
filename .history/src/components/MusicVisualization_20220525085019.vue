...
export default {
    ...
    watch: {
        currentAudio: this.$store.state.globalPlayer.cloneNode(true),
    },
    methods: {
        visualizerSelected() {
            // 更新当前可视化选项
        ...
        musicVisualization() {
            ...
            // 深复制 DOM 元素
            // const currentAudio = this.$store.state.globalPlayer.cloneNode(true);
            // 创建音频源 MediaElementAudioSourceNode (currentAudio 作为音频源)
            const source = audioCtx.createMediaElementSource(this.currentAudio);
            // 将音频源关联到分析器
