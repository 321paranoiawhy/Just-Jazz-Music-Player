<template>
    ...
    <div class="container">
        ...
        <div class="range-input">
            ...
            <!-- 显示当前音量 (百分比, 默认 100%) -->
            <p class="p-right" ref="playerVolume">
                {{ Math.floor(player.volume * 100) }}%
            </p>
        </div>
        <!-- <p ref="playerVolume">{{ player.volume }}</p> -->
    ...
    methods: {
        ...
        mute() {
            if (this.player.volume) {
                ...
                console.log(this.$refs.playerVolume.value);
                this.$refs.playerVolume.value = 0;
                console.log(this.$refs.playerVolume.value);
            } else {
                // 如果已静音则恢复音量为静音前音量
                this.player.volume = this.beforeMutedVolume;
                this.$refs.playerVolume.value = this.beforeMutedVolume;
            }
        },
        adjustVolume(data) {
            // 调节音量 (0-1)
