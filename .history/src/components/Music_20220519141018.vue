...
export default {
    ...
    mounted() {
        ...
        this.player.addEventListener("timeupdate", () => {
            ...
            this.$refs.percent.title = this.currentTime;
        });
        // 当前歌曲结束
        // TODO
        this.player.addEventListener("ended", () => {
            switch (this.playMode) {
                ...
                case 2:
                    ...
                    let randomIndex =
                        Math.floor(Math.random() * this.musicData.length) + 2;
                    while (randomIndex === this.currentSongIndex) {
                        randomIndex =
                            Math.floor(Math.random() * this.musicData.length) +
                            2;
                    }
    ...
    //         // console.log(typeof this.totalTime);
    //         // console.log(Number(this.currentTime) / Number(this.totalTime));
    //         return Number(this.currentTime) / Number(this.totalTime);
    //     },
    // },
    methods: {
        changeStyle(index) {
        ...
        changeCurrentTime(parameter) {
            // // 更改 currentTime 的值为 入参的值
            // this.currentTime = parameter;
            // 更改 player.currentTime 的值为 入参的值
            this.player.currentTime = parameter * this.totalTime;
